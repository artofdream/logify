...
| Sensors | Go tests, fixtures, `go vet`, build, `git diff --check`, multi-OS CI, Node `--check` and `followup.js` store tests | Partial | `test` matrix on Ubuntu/Windows/macOS plus `validate` aggregator on `main` push/PR; local commands pass; `TestFollowUpStoreCreateExportImport` executes the follow-up store | No docs/link checker, benchmark, accessibility sensor, or full in-browser DOM/page execution |
