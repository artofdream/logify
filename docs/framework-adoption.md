...
| Sensors | Go tests, fixtures, `go vet`, build, `git diff --check`, multi-OS CI, Node `--check`, `followup.js` store tests, NFR-021 source-contract and 10k `store.filter` probe | Partial | `test` matrix on Ubuntu/Windows/macOS plus `validate` aggregator on `main` push/PR; local commands pass; `TestFollowUpStoreCreateExportImport` executes the follow-up store; `TestNFR021IssueWorkflowA11yContracts` and `TestNFR021IssueFilterProbe` run the NFR-021 harnesses | No docs/link checker, analyzer benchmark, WCAG engine / AT run, or full in-browser DOM/page execution |
